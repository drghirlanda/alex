...
presentations of each of two stimuli (S1s) called Red and White. Red
will be rewarded 90\% of the time, White only 10\% (specified by
...
S2 field can be left empty if probability of S2 presentation is 0.
...
\begin{longtable}[c]{@{}lllll@{}}
\caption{With this \texttt{Phases.csv} file, Phase 1 will be run first.
...
\begin{longtable}[c]{@{}lllll@{}}
\caption{With this \texttt{Phases.csv} file, Phase 2 will be run first.
...
higher-order conditioning), and where S2 would not be considered a
reward.
...
\item
  ...
  \item
    ...
    folder. An optional zoom factor, separated from the filename by a
    \texttt{+} (plus), can be provided to scale an image to a desired
    size. The following stylized faces (smileys) come with alex and you
    can use them without having them in the Materials folder:
...
\item
  ...
  RGB triplets, delimited by hyphens (\texttt{-}). In the case of text,
  you can specify the background as well as the foreground color by
  writing the color in the form Color1+Color2, where Color1 is the
...
\item
  ...
  except for Smiley, which is displayed 150 pixels above center
  (negative Y values place stimuli above center, negative X values place
  them left of center).
\end{itemize}
...
specify one group and its size. The file in Table \ref{subjects}
...
subjects need to be divided into different treatment groups. If you want
to test, say, two shades of pink, you would extend the
\texttt{Stimuli.csv} file in Table \ref{stimuli-color}. The special
value \texttt{*} here indicates that the color of stimulus Pink will be
looked up, for each subject, in the column PinkColor of the
\texttt{Groups.csv} file (Table \ref{subjects-color}). This syntax is
available for all stimulus properties. For example, to vary the size of
the Red square across subjects you would use the \texttt{Groups.csv} and
\texttt{Stimuli.csv} files in Tables \ref{subjects-color-parameters} and
...
\caption{A \texttt{Groups.csv} file instructing alex to run 20 subjects
...
experimental groups, each receiving a unique combination of PinkColor
and RedParameters (see Table \ref{stimuli-color-parameters}).
...
the whole experiment. Here is a sample file:
...
\begin{itemize}
\item
  \textbf{S1Duration} is the default duration of all S1 stimuli, while
  \textbf{S2Duration} is the default duration of all S2 stimuli. All
...
\item
  ...
  want to respond. Note that this can also be set on a per-trial basis,
  see \hyperref[responses]{here}. \label{maxresponses}
\item
  ...
  different trial types, by including a MaxResponses column in
  \texttt{Phases.csv} (see the \hyperref[textfiles]{section on text
  files} for an example). If a MaxResponses column exists, but the value
  is empty for some stimuli, the MaxResponses value in
...
may also be used to present a combination of visual and auditory
stimuli.
...
groups by using the \texttt{*} and \texttt{:} notation. For example,
imagine that we want to investigate how discrimination learning proceeds
as a function of reward probability. We could use the
...
that the value of S2Prob for Training trials with S1 A must be looked up
in \texttt{Groups.csv} (see Table \ref{groups-phases-star-notation}).
...
Note that the name of the corresponding column in \texttt{Groups.csv} is
...
refers (in employing the same notation for stimuli, we had to worry only
about the stimulus name). This works also to set phase parameters for a
compound stimulus. For example, if you want to set the \texttt{S2Prob}
value for S1 \texttt{A+B}, you would use the column
...
key is the correct response for S1 Red in Phase 1, but that the right
arrow is correct for White, you would write as in Table
\ref{per-stimulus-responses}.

\begin{longtable}[c]{@{}llllll@{}}
\caption{A \texttt{Phases.csv} specifying different responses for S1s
Red and White in Phase 1. \label{per-stimulus-responses}}\tabularnewline
\toprule
...
parameters}). The S2 (if any) is presented only once at the end of the
trial, so it is irrelevant when a subject responds.
...
\texttt{Phases.csv}, which makes sure the user only has to press the
space bar (the default response) once to move on, even if a larger
number of responses is allowed for actual experimental trials.
...
\toprule
Phase & S1 & Trials & MaxResponses\tabularnewline
\midrule
\endfirsthead
\toprule
Phase & S1 & Trials & MaxResponses\tabularnewline
...
necessary files, are reported only on the standard output. When running
alex through the PEBL launcher, these messages will appear in files
\texttt{stdout.txt} and \texttt{stderr.txt}, which PEBL creates in the
folder where alex is run.
