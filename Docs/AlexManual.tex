...
\textbf{S1Duration} is the default duration of all the S1 stimuli, while
...
window are considered invalid (see \hyperref[data-format]{Data format})
and preclude S2 presentation. If not specified, ResponseTimeMin is set
to 0 and ResponseTimeMax to S1Duration, thus allowing responses at any
...
\textbf{Log} is a toggle for the logging feature, which records runtime
messages for each subject in files named the same as corresponding data
files, except with a \texttt{.log} suffix. Disabled by setting to 0.
